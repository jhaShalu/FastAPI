...
BOOKS = [
    Book(1, 'Computer Science Pro', 'codingwithruby', 'A very nice book!', 5, 2001),
    Book(2, 'Be Fast with FastAPI', 'codingwithjayanth', 'A great book!', 5, 2002),
    Book(3, 'Master Endpoints', 'codingwithme', 'A awesome book!', 5, 2003),
    Book(4, 'HP1', 'Author1', 'Book Description1', 3, 2004),
    Book(5, 'HP2', 'Author2', 'Book Description2', 2, 2005),
    Book(6, 'HP3', 'Author3', 'Book Description3', 1, 2006)
]
...
async def update_book(updated_book: BookRequest):
    ...
    for book_index in range(len(BOOKS)):
        if BOOKS[book_index].id == updated_book.id:
            BOOKS[book_index] = Book(**updated_book.model_dump())
            book_changed = True
    if not book_changed:
        raise HTTPException(status_code=404, detail="Item not found!!")
